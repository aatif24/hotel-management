"use client";

import { Button, Card, Chip, cn } from "@heroui/react";
import { HugeiconsIcon } from "@hugeicons/react";
import {
  MoneyExchange01Icon,
  SendingOrderFreeIcons,
  SquareArrowDownDoubleIcon,
  SquareArrowRightDoubleIcon,
  SquareArrowUpDoubleIcon,
  Wallet,
} from "@hugeicons/core-free-icons";

const data = [
  {
    title: "Total Orders",
    value: "540",
    change: "33%",
    rupee:false,
    changeType: "positive",
    trendChipPosition: "top",
    iconName: SendingOrderFreeIcons,
  },
  {
    title: "Total Sales",
    value: "15,400",
    change: "0.0%",
    changeType: "neutral",
    trendChipPosition: "top",
    rupee:true,
    iconName: Wallet,
  },
  {
    title: "Net Profit",
    value: "10,400",
    change: "3.3%",
    changeType: "negative",
    trendChipPosition: "top",
    rupee:true,
    iconName: MoneyExchange01Icon,
  },
];

export default function StatsComponent() {
  return (
    <dl className="grid w-full grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3">
      {data.map(
        (
          { title, value, change, changeType, iconName, trendChipPosition, rupee },
          index,
        ) => (
          <Card
            key={index}
            className="dark:border-default-100 border border-transparent"
          >
            <div className="flex p-4">
              <div
                className={cn(
                  "mt-1 flex h-8 w-8 items-center justify-center rounded-md",
                  {
                    "bg-success-50": changeType === "positive",
                    "bg-warning-50": changeType === "neutral",
                    "bg-danger-50": changeType === "negative",
                  },
                )}
              >
                {changeType === "positive" ? (
                  <HugeiconsIcon
                    className="text-success"
                    icon={iconName}
                    width={20}
                  />
                ) : changeType === "neutral" ? (
                  <HugeiconsIcon
                    className="text-warning"
                    icon={iconName}
                    width={20}
                  />
                ) : (
                  <HugeiconsIcon
                    className="text-danger"
                    icon={iconName}
                    width={20}
                  />
                )}
              </div>

              <div className="flex flex-col gap-y-2">
                <dt className="text-small text-default-500 mx-4 font-medium">
                  {title}
                </dt>
                <dd className="text-default-700 px-4 text-2xl font-semibold">
                  {rupee ? <span>&#8377;</span> : null}
                  {value}
                </dd>
              </div>

              <Chip
                className={cn("absolute right-4", {
                  "top-4": trendChipPosition === "top",
                  "bottom-4": trendChipPosition === "bottom",
                })}
                classNames={{
                  content: "font-semibold text-[0.65rem]",
                }}
                color={
                  changeType === "positive"
                    ? "success"
                    : changeType === "neutral"
                      ? "warning"
                      : "danger"
                }
                radius="sm"
                size="sm"
                startContent={
                  changeType === "positive" ? (
                    <HugeiconsIcon
                      height={12}
                      icon={SquareArrowUpDoubleIcon}
                      width={12}
                    />
                  ) : changeType === "neutral" ? (
                    <HugeiconsIcon
                      height={12}
                      icon={SquareArrowRightDoubleIcon}
                      width={12}
                    />
                  ) : (
                    <HugeiconsIcon
                      height={12}
                      icon={SquareArrowDownDoubleIcon}
                      width={12}
                    />
                  )
                }
                variant="flat"
              >
                {change}
              </Chip>
            </div>

            <div className="bg-default-100">
              <Button
                fullWidth
                className="text-default-500 flex justify-start text-xs data-pressed:scale-100"
                radius="none"
                variant="light"
              >
                View All
              </Button>
            </div>
          </Card>
        ),
      )}
    </dl>
  );
}
